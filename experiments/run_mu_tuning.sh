...
N_ROUNDS=20
smpc_subdir=""
param_tuning_res="${root_dir}/output/annotation/mu-tuning-res"
if [ "$smpc" == "true" ]; then
    smpc_subdir="/smpc"
...
for key in "${!datasets[@]}"; do
    for epoch in 1 2 3 4 5; do
      for mu in 0.001 0.01 0.05 0.1 0.2 0.5; do
          echo -e "\e[32m******************************************\e[0m"
          echo -e "\e[32mRunning annotation on ${key} dataset for mu=${mu}\e[0m"
          echo -e "\e[32m******************************************\e[0m"
          ...
          output="${root_dir}/output/annotation/${args[0]}/mu_tuning${smpc_subdir}"
